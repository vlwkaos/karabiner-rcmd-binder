...
fn handle_normal_mode(app: &mut App, key: KeyCode, _modifiers: KeyModifiers, tx: &Sender<Vec<String>>) -> Result<()> {
    match key {
        KeyCode::Char('q') => {
...
fn handle_bindings_normal(app: &mut App, key: KeyCode, tx: &Sender<Vec<String>>) -> Result<()> {
    match key {
        ...
        match key {
            ...
            KeyCode::Enter => {
                ...
                if app.show_autocomplete && action_editor.field == ActionEditorField::Target {
                    if let Some(selected) = app.autocomplete_suggestions.get(app.autocomplete_selected) {
                        action_editor.target = selected.clone();
                    }
                    app.show_autocomplete = false;
                }
            }
            ...
            KeyCode::Char('<') | KeyCode::Char(',') if action_editor.field != ActionEditorField::Target => {
    ...
    match key {
        ...
        KeyCode::Enter => {
            ...
            if app.show_autocomplete {
                if let Some(selected) = app.autocomplete_suggestions.get(app.autocomplete_selected).cloned() {
...
fn handle_key_field(app: &mut App, key: KeyCode) -> Result<()> {
    match key {
        ...
        KeyCode::Right => {
            // Accept autocomplete
            if let Some(selected) = app.select_autocomplete() {
                if let Some(editor) = app.binding_editor.as_mut() {
                    editor.key = selected;
                }
                app.show_autocomplete = false;
...
fn spawn_app_discovery(tx: Sender<Vec<String>>) {
    thread::spawn(move || {
