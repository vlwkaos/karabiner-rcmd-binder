...
const RULE_PREFIX: &str = "[switchkey]";
const VAR_PREFIX: &str = "switchkey_";
...
fn generate_binding_rule(binding: &Binding, default_browser: &Browser, scripts_dir: &str) -> Value {
    let description = if binding.description.is_empty() {
        ...
        format!("{} rcmd+{}: {}", RULE_PREFIX, binding.key, binding.description)
    };
...
fn generate_single_action_manipulators(
    ...
    let to = if binding.actions.is_empty() {
        ...
        vec![action_to_karabiner(&binding.actions[0], default_browser, scripts_dir)]
    };
...
fn action_to_karabiner(action: &Action, default_browser: &Browser, scripts_dir: &str) -> Value {
    match action {
        Action::App { target } => {
            // Use native open_application for single apps
            json!({
                "shell_command": format!("open -a '{}'", target)
            })
        }
        Action::Url {
            target,
...
/// This preserves existing rules and only replaces our [switchkey] rules
pub fn apply_to_karabiner(config: &Config, scripts_dir: &str) -> Result<()> {
    ...
    if profile.get("complex_modifications").is_none() {
        profile.as_object_mut().unwrap().insert(
            "complex_modifications".to_string(),
            json!({ "rules": [] }),
        );
    }
    ...
    // Remove existing [switchkey] rules
    rules.retain(|rule| {
...
/// Remove all [switchkey] rules from karabiner.json
pub fn remove_from_karabiner() -> Result<()> {
    let config_path = karabiner_config_path()?;
    ...
    let content = fs::read_to_string(&config_path)?;
    let mut kara_config: Value = serde_json::from_str(&content)?;

    // Create backup before modifying
    create_backup()?;

    if let Some(profiles) = kara_config.get_mut("profiles").and_then(|p| p.as_array_mut()) {
